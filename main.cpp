...
const char *usage =
    ...
    "\t-preprocessing_method [octree,none]  Preprocessing method\n"
    "\t-octree_target_size <size>   OcTree Target Size\n"
    "\t-octree_denoise <number_of_points> Points that are in a box with less than or equal number_of_points points, are labelled noise\n"
    ...
    "\t-mst_min_cluster_size <min_cluster_size> All cluster with size<min_cluster_size are considered noise. Default: 2\n"
    "\t-mst_max_mean_edge_len <max_mean_edge_len> Helps with noise reduction with umst method. Default -1 (not used)\n"
    "\t-generate_mst_debug_gnuplot <filename> Generate gnuplot file for the generated mst\n\n"
